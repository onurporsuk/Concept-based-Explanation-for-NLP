"""
This module implements a GPT2 model with a non-negative (ReLU) last layer to allow us to discover
...
class CustomGPT2ClassificationHead(nn.Module):
    """
    A custom fully-connected classification head for GPT2 with a non-negative layer on which
    ...
    def __init__(self, config):
        ...
        classifier_dropout = (
            config.attn_pdrop if config.attn_pdrop is not None else config.summary_first_dropout
        )
...
class CustomGPT2ForSequenceClassification(GPT2PreTrainedModel):
    """
    A custom GPT2 model using a custom fully-connected head with a non-negative layer on which
    we can compute the NMF.
    ...
    def __init__(self, config):
        ...
        self.classifier = CustomGPT2ClassificationHead(config)
